...
class InjectiveFlow(Flow):
    """Base class for all injective flow objects."""

    def __init__(self, embedding, distribution, conformal, embedding_net=None):
        super().__init__(embedding, distribution, embedding_net)
        self.embedding = embedding
        self.conformal = conformal # Whether the embedding/transform is conformal
        self.distribution = distribution

    def reconstruct(self, inputs):
        mid_latent, _ = self.embedding.forward(inputs)
        reconstruction, _ = self.embedding.inverse(mid_latent)
        return reconstruction

    def reconstruct_and_log_prob(self, inputs, context=None):
        '''If conformal, return the log likelihood; otherwise, return
           log likelihood given by the base distribution
        '''
        mid_latent, _ = self.embedding.forward(inputs)
        reconstruction, log_conf_det = self.embedding.inverse(mid_latent)

        log_pu = self.distribution.log_prob(mid_latent, context)
        log_prob = log_pu - log_conf_det if self.conformal else log_pu
