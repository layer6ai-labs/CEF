...
class MultiscaleCompositeTransform(Transform):
    ...
    def __init__(self, num_transforms, split_dim=1, out_shape=None):
        """Constructor.
    ...
    def forward(self, inputs, context=None):
        ...
        for outputs, logabsdet in cascade():
            if self.out_shape is not None:
                all_outputs.append(outputs.reshape(batch_size, -1, *self.out_shape))
            else:
    ...
    def inverse(self, inputs, context=None):
        if inputs.dim() != 2 and self.out_shape is None:
            raise ValueError("Expecting NxD inputs")
        elif inputs.dim() != 4 and self.out_shape is not None:
            raise ValueError("Expecting NxD inputs")
        if self._num_transforms != len(self._transforms):
            raise RuntimeError(
                "Expecting exactly {} transform(s) "
        ...
        if self.out_shape is not None:
            shape_factor = np.prod(self.out_shape)
            split_indices = np.cumsum([np.prod(shape) // shape_factor
        ...
        for i in range(len(self._output_shapes)):
            ...
            flat_input.reshape(batch_size, -1)
